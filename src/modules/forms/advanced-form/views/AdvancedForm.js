...
import styles from './AdvancedForm.less';
...
class AdvancedForm extends PureComponent {
  ...
  render() {
    ...
    const validate = () => {
      validateFieldsAndScroll((error, values) => {
        if (!error) {
          ...
          dispatch({
            type: 'advancedForm/submitAdvancedForm',
            payload: values
          });
        }
      });
    ...
    return (
      <div>
        <Card title="仓库管理" className={styles['sg-form_card']} bordered={false}>
...
const mapStateToProps = ({ global, loading }) => ({
  ...
  submitting: loading.effects['advancedForm/submitAdvancedForm']
});

const title = '高级表单';
const content = '高级表单常见于一次性输入和提交大批量数据的场景。';
export default wrapWithPageHeaderLayout(connect(mapStateToProps)(Form.create()(AdvancedForm)), { title, content });
